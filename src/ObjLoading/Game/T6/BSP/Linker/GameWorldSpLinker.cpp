...
namespace BSP
{
    GameWorldSpLinker::GameWorldSpLinker(MemoryManager& memory, ISearchPath& searchPath, AssetCreationContext& context)
        : m_memory(memory),
    ...
    GameWorldSp* GameWorldSpLinker::LinkGameWorldSp(const BSPData& bsp) const
    {
        return LinkGameWorld<GameWorldSp>(m_memory, bsp);
    }
} // namespace BSP
