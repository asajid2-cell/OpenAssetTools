...
namespace BSP
{
    namespace
    {
        constexpr const char* FALLBACK_SURFACE_MATERIAL = "streaming_temp_image_0";
        constexpr const char* FALLBACK_IMAGE_NAMES[] = {
            "reflection_probe0",
            "lightmap0_secondary",
        };
    }
    ...
    bool GfxWorldLinker::LoadMapSurfaces(const BSPData& bsp, GfxWorld& gfxWorld) const
    {
        ...
        for (size_t surfIdx = 0; surfIdx < surfaceCount; surfIdx++)
        {
            ...
            // Minimal seed-map imports commonly come through FBX as a bare lambert1 surface.
            // For the loader-validation path, bind those surfaces to a stock common_zm temp material
            // instead of trying to resolve a full authored shader stack.
            if (surfMaterialName == "lambert1")
                surfMaterialName = FALLBACK_SURFACE_MATERIAL;
            ...
            if (!surfMaterialAsset)
            {
                ...
                if (!surfMaterialAsset)
                    surfMaterialAsset = m_context.LoadDependency<AssetMaterial>(FALLBACK_SURFACE_MATERIAL);
                if (!surfMaterialAsset)
                {
                    con::error("unable to load a fallback surface material for {}!", surfMaterialName);
                    return false;
                }
            }
    ...
    void GfxWorldLinker::LoadLightGrid(GfxWorld& gfxWorld) const
    {
        ...
        memset(gfxWorld.lightGrid.colors, BSPEditableConstants::LIGHTGRID_COLOUR, gfxWorld.lightGrid.colorCount * sizeof(GfxCompressedLightGridColors));
    ...
    bool GfxWorldLinker::LoadReflectionProbeData(GfxWorld& gfxWorld) const
    {
        ...
        if (!probeImageAsset)
        {
            for (const auto* fallbackImageName : FALLBACK_IMAGE_NAMES)
            {
                if (probeImageAsset)
                    break;
                if (probeImageName == fallbackImageName)
                    continue;
                probeImageAsset = m_context.LoadDependency<AssetImage>(fallbackImageName);
            }
            if (!probeImageAsset)
            {
    ...
    void GfxWorldLinker::LoadSkyBox(const BSPData& projInfo, GfxWorld& gfxWorld) const
    {
        ...
        gfxWorld.skyBoxModel = m_memory.Dup("");
        const auto skyboxModelFile = std::format("xmodel/{}.json", skyBoxName);
        const auto skyboxModel = m_search_path.Open(skyboxModelFile);
        if (skyboxModel.IsOpen())
        {
            const auto skyboxModelAsset = m_context.LoadDependency<AssetXModel>(skyBoxName);
            if (skyboxModelAsset)
                gfxWorld.skyBoxModel = m_memory.Dup(skyBoxName.c_str());
            else
                con::warn("WARN: Unable to load the skybox xmodel {}, continuing without skybox model.", skyBoxName);
        }
        else
        {
            con::warn("WARN: No custom-map skybox xmodel source found for {}, continuing without skybox model.", skyBoxName);
        }
    ...
    bool GfxWorldLinker::LoadOutdoors(GfxWorld& gfxWorld) const
    {
        ...
        {
            auto fallbackOutdoorImageAsset = outdoorImageAsset;
            for (const auto* fallbackImageName : FALLBACK_IMAGE_NAMES)
            {
                fallbackOutdoorImageAsset = m_context.LoadDependency<AssetImage>(fallbackImageName);
                if (fallbackOutdoorImageAsset)
                    break;
            }
            if (!fallbackOutdoorImageAsset)
            {
            ...
            gfxWorld.outdoorImage = fallbackOutdoorImageAsset->Asset();
            return true;
        }
    ...
    GfxWorld* GfxWorldLinker::LinkGfxWorld(const BSPData& bsp) const
    {
        ...
        {
            std::fprintf(stderr, "[oat][gfxworld] %s\n", stage);
            std::fflush(stderr);
        };
